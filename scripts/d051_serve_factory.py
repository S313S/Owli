"""§D-051 货 5 真机的服务入口（端口 8975，本 worktree 的新代码）。

抄 `scripts/d047_serve_factory.py`，只换库名与端口。起法（在 worktree 根目录）：
    OWLI_AUTO_CONFIRM=1 nohup ../Owli/.venv/bin/python -m uvicorn \\
        scripts.d051_serve_factory:app --factory --port 8975 > var/serve-8975.log 2>&1 &
沙盒怎么来（别 cp 活库，8956 正拿着它当运行时）：
    sqlite3 ../Owli-mvp/var/mvp-8956.db ".backup $(pwd)/var/d051-sandbox.db"
...
def app():
    return create_app(
        database_path=ROOT / "var" / "d051-sandbox.db",
