"""§D-046 货 4 真机的服务入口（端口 8968，本 worktree 的新代码）。

起法（在 worktree 根目录）：
    OWLI_AUTO_CONFIRM=1 nohup ../Owli/.venv/bin/python -m uvicorn \\
        scripts.d046_serve_factory:app --factory --port 8968 > var/serve-8968.log 2>&1 &
沙盒怎么来（别 cp 活库，8956 正拿着它当运行时）：
    sqlite3 ../Owli-mvp/var/mvp-8956.db ".backup $(pwd)/var/d046-sandbox.db"
...
def app():
    return create_app(
        database_path=ROOT / "var" / "d046-sandbox.db",
