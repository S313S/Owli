...
def _task(body: str, output_path: Path, research_id: str, model: str) -> EngineTask:
    return EngineTask(
        ...
        research_id=research_id, goal_id="polished", agent_id=AGENT_ID,
...
async def polish(store: Any, research_id: str, runs_root: Path, report_text: str, *,
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        if not offpool:
            return {"status": "ok", "template": skill.name, "path": str(md_path),
                    "tables_path": str(tables_path), "attempts": attempt, "offpool": []}
    ...
    return {"status": "failed", "template": skill.name, "path": str(md_path),
            "tables_path": str(tables_path), "attempts": MAX_ATTEMPTS,
            "offpool": offpool_marks(md_path.read_text(encoding="utf-8"), pool)
            if md_path.is_file() else [], "errors": list(errors)}
