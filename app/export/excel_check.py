...
降级规则（本包自拍，worklog §3）：G–K 某格为空 = 该维不可评（库里 NULL、
rating_notes 记 `?`），此时 F 列须为 `? / ?`、L 列该段须为 `?`；五维全空且无 L 视为未评级。
五维齐全时按 spec 严格断言 F = 和、等级阈值、L 各段 = G–K。
"""
...
def _check_sources(ws) -> list[str]:
    """第 5/7/8 项：D 列超链接；G–K 0–2 整数、F=和且等级一致；L 与 G–K 对应。"""
    errors: list[str] = []
    for r in range(2, ws.max_row + 1):
        mark = ws.cell(row=r, column=1).value
        if not mark:
            continue
        url = ws.cell(row=r, column=4)
        if not url.value or not url.hyperlink:
            errors.append(f"5 行{r} D 列非超链接或为空")
        scores = [ws.cell(row=r, column=c).value for c in range(7, 12)]
        summary = str(ws.cell(row=r, column=6).value or "")
        notes = str(ws.cell(row=r, column=12).value or "")
        bad = [s for s in scores if s is not None and (not isinstance(s, int) or isinstance(s, bool) or not 0 <= s <= 2)]
        if bad:
            errors.append(f"7 行{r} G–K 含非 0–2 整数: {scores}")
            continue
        if any(s is None for s in scores):  # 某维不可评（`?` 记法）：F 列须为 `? / ?`，不重算
            if summary != "? / ?":
                errors.append(f"7 行{r} 有维度未评但 F 列不是 '? / ?': {summary}")
            if all(s is None for s in scores) and not notes:
                continue  # 完全未评级：L 允许为空
        else:
            total = sum(scores)
            matched = SUMMARY.match(summary)
            if not matched or matched.group(1) != str(total) or matched.group(2) != _grade(total):
                errors.append(f"7 行{r} F 列与 G–K 不一致: {summary!r} vs {total}/{_grade(total)}")
        parsed = NOTES.match(notes)
        if not parsed:
            errors.append(f"8 行{r} L 列不合 rating_notes 正则: {notes[:40]!r}")
        elif [parsed.group(i) for i in (1, 3, 5, 7, 9)] != ["?" if s is None else str(s) for s in scores]:
            errors.append(f"8 行{r} L 列各段分数与 G–K 不等")
    return errors
