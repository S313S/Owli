...
def _shard_notice(
    ...
    notice = (
        ...
        "要说范围就说「本节」「本次样本」。\n"
        # §D-051 货 6：可看性尺子 R7 的词表比上面这句宽——它还禁运行时章号与
        # `shard`。写手照旧在『证据缺口』里点名缺的是哪一章，用的就是那串编号
        #（真机 r-13d61d236bff 原文：「缺失章 goal-3/ch-3、goal-1/ch-1 的搜索
        # 抓取因 timeout 未落地」）。这句话早于本卡（底料自己那份第 1 片里就有
        # 两处），8956 那轮红不出来只是因为第 1 片挂了、这三段整个没进成稿；
        # 本卡把片救回来，它就跟着回来了。提示词与尺子对齐到这里。
        "**同样不要写 `goal-1/ch-1`、`ch-6/sec-1` 这类运行时章节编号，"
        "也不要写 `shard`、HTML 注释**——那是系统调度用的内部坐标，"
        "读者手上那份报告里根本没有这个号，看到只会一头雾水。"
        "要说别处缺了什么，就按**内容**讲（如「官方定价页面这一块没取到」），"
        # §D-052 货 1：上面那句原文是「不要给编号」。A/B 造红实证，写手把它读成
        # 连证据角标也别给——含这句的两轮各漏 9 条 / 3 条证据，剪掉它的两轮零漏。
        # 这里把管辖范围点死：它管的是章节编号，不管证据角标。
        "不要给章节编号。**这一条只管章节编号，不管证据角标**——"
        "`[S01]` 这类角标是读者查证的唯一入口，该引的一个都不许省。\n"
    )
    if index == 1:
        notice += (
    ...
    notice += (
        ...
        "3. 不写节标题行、不写总起段与收束段。\n"
        # §D-052 货 1：原来三条硬约束全是**上限**（最多几条、只引池内、不写什么），
        # 没有一条要求「池里每条都要用」，写手照着写完 5 条就交卷，池里另外 3 条
        # 一个字没提也不违规。少引的证据不进 `## 信息源`，读者那份报告里就等于
        # 从没采过——21/30 那轮的九条就是这么没的。补一条**下限**。
        "4. 本片池里的**每一条**证据都要在 `## 信息源` 出现，且至少被一条 "
        "`## 结论` 列表项引用；一条结论可以带多个角标，与第 1 条的条数上限不冲突。"
        "某条确实与本节无关，就在结论里写明为什么不采信，同样带上它的角标。\n"
    )
    if index == 1:
        notice += "5. 全节口径的三段合计不超过 6 行。\n"
    return notice
